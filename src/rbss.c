				...
				double *lambda, int n, int p, int q, int nlam, double tau, int criteria)
{
	...
	for(k=0;k<nlam;k++){
		...
		switch (criteria){
		case 1: bic[k] = log(tmp/n/q) + 2*pow(1.0*df,tau)/n/q; 	break; 	// AIC
		case 2: bic[k] = log(tmp/n/q) + log(n*q)*df/n/q; 		break;	// BIC
...
void PvaluesMarginal(double *Tn, double *sigma2, double *x, double *y, int n, int p, int q){
	// input:
	// x in R^{p*n}
	// y in R^{q*n}
...
void PvaluesMarginal_bic(double *Tn, double *sigma2, double *x, double *y, int n, int p, int q, int nlam, double *gamma){
	// input:
	// x in R^{p*n}
	// y in R^{q*n}
	...
	for(j=0;j<q;j++){
		...
		Tn[j] 	= tmp1/s;
		for(k=0;k<nlam;k++)
			sigma2[j*nlam+k] = pow(s,(gamma[k]-1.0)/gamma[k]);
	}
	free(Q);
	free(R);
	free(qy);
...
SEXP PVALUES(SEXP X_, SEXP Y_, SEXP DIM_){
	...
	// Outcome
	SEXP rTn, rSigma2, list, list_names;
  	PROTECT(rTn 		= allocVector(REALSXP, 	q));
	PROTECT(rSigma2 	= allocVector(REALSXP, 	q));
	PROTECT(list_names 	= allocVector(STRSXP, 	2));
	PROTECT(list 		= allocVector(VECSXP, 	2));

	PvaluesMarginal(REAL(rTn), REAL(rSigma2), x, y, n, p, q);
...
SEXP PVALUES_BIC(SEXP X_, SEXP Y_, SEXP DIM_, SEXP GAMMA_){
	int *dim      	= INTEGER(DIM_);
	int n         	= dim[0];
	int p     	  	= dim[1];
	int q         	= dim[2];
	...
	SEXP rTn, rSigma2, list, list_names;
  	PROTECT(rTn 		= allocVector(REALSXP, 	q));
	PROTECT(rSigma2 	= allocVector(REALSXP, 	q*nlam));
	PROTECT(list_names 	= allocVector(STRSXP, 	2));
	PROTECT(list 		= allocVector(VECSXP, 	2));

	PvaluesMarginal_bic(REAL(rTn), REAL(rSigma2), x, y, n, p, q, nlam, gamma);
